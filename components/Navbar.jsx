'use client';

import { motion } from 'framer-motion';
import styles from '../styles';

import { navVariants } from '../utils/motion';

const Navbar = () => (
  <motion.nav variants={navVariants} initial="hidden" whileInView="show" className={`${styles.xPaddings} py-8 relative`}>
    <div className="absolue w-[50%] inset-0 gradient-01" />
    <div className={`${styles.innerWidth} mx-auto flex gap-8 justify-center`}>
      {/* <img src="/search.svg" alt="search" className="w-[24px] h-[24px] object-contain" /> */}
      <h2 className="font-extrabold text-[34px] leading-[30px] text-white">Portfolio</h2>
      {/* <img src="/menu.svg" alt="menu" className="w-[34px] h-[34px] object-contain" /> */}
    </div>

  </motion.nav>
);

export default Navbar;
